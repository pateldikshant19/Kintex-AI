...
const footballPlayers = [
  ...
  { name: 'Jules Koundé', position: 'Defender', jersey: 23 }
];
...
const seedTeamPlayers = async () => {
  try {
    ...
    for (const team of cricketTeams) {
      ...
      for (const playerData of cricketPlayers) {
        const player = new Player({
          name: playerData.name,
          email: `${playerData.name.toLowerCase().replace(/\s+/g, '.')}.${emailCounter}@cricket.com`,
          sport: 'Cricket',
          teamName: team,
          ...
          playingStyle: ['Aggressive', 'Defensive', 'Tactical', 'Balanced'][Math.floor(Math.random() * 4)],
          debutSeason: `${2015 + Math.floor(Math.random() * 8)}`,
          internationalTeam: ['India', 'Australia', 'England', 'South Africa', 'New Zealand'][Math.floor(Math.random() * 5)],
          weaknesses: ['Short-pitch bowling', 'Left-arm spinners', 'Pressure situations'][Math.floor(Math.random() * 3)],
      ...
      const result = await Player.insertMany(playersToInsert);
    ...
    for (const team of footballTeams) {
      ...
      for (const playerData of footballPlayers) {
        const player = new Player({
          name: playerData.name,
          email: `${playerData.name.toLowerCase().replace(/\s+/g, '.')}.${emailCounter}@football.com`,
          sport: 'Football',
          teamName: team,
          position: playerData.position,
          jerseyNumber: playerData.jersey,
          ...
          playingStyle: ['Attacking', 'Defensive', 'Balanced', 'Technical'][Math.floor(Math.random() * 4)],
          debutSeason: `${2015 + Math.floor(Math.random() * 8)}`,
          internationalTeam: ['Spain', 'Brazil', 'France', 'Germany', 'Argentina', 'England'][Math.floor(Math.random() * 6)],
          weaknesses: ['Heading', 'Left foot', 'Speed recovery', 'Physical strength'][Math.floor(Math.random() * 4)],
    ...
    for (const contingent of trackAndFieldContingents) {
      ...
      for (const playerData of contingent.players) {
        const player = new Player({
          name: playerData.name,
          email: `${playerData.name.toLowerCase().replace(/\s+/g, '.')}.${emailCounter}@trackfield.com`,
          sport: 'Track & Field',
          ...
          playingStyle: ['Explosive', 'Endurance', 'Technical', 'Balanced'][Math.floor(Math.random() * 4)],
          debutSeason: `${2015 + Math.floor(Math.random() * 8)}`,
          internationalTeam: contingent.country,
          weaknesses: ['Starting blocks', 'Cornering', 'Finishing', 'Mental focus'][Math.floor(Math.random() * 4)],
